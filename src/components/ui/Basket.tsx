...
export default function Basket() {
  const { totalPrice, selectedBets }  = useContext(BetCouponContext);

  return (
    <section className="block fixed right-0 bottom-0 py-2 px-5 m-0 w-[calc(500px)] text-sm leading-3 align-baseline bg-white text-neutral-400 shadow-2xl shadow-neutral-600 rounded-tl-lg border">
      ...
      <p className="py-4 px-0 m-0 leading-3 align-baseline text-right">
        Toplam: <b>{totalPrice}</b>
      </p>
    </section>
  );
}